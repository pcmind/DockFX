  ...
  public void handle(MouseEvent event)
  {
    ...
    else if (event.getEventType() == MouseEvent.DRAG_DETECTED)
    {
      if (!dockNode.isFloating())
      {
        ...
        // Temporary work around for nodes losing the drag event when removed
        // from
        // the scene graph.
        ...
        // However since flickering when popping out was already eliminated that
        // would
      ...
      else if (dockNode.isMaximized())
      {
        ...
        // Please note that setMaximized is ruined by width and height changes
        // occurring on the
        // stage and there is currently a bug report filed for this though I did
        // not give them an
        // accurate test case which I should and wish I would have. This was
        // causing issues in the
        // original release requiring maximized behavior to be implemented
        // manually by saving the
    ...
    else if (event.getEventType() == MouseEvent.MOUSE_DRAGGED)
    {
      ...
      Insets insetsDelta = this.getDockNode()
                               .getBorderPane()
                               .getInsets();

      // it is possible that drag start has not been set if some other node had
      // focus when
      // we started the drag
      if (null == dragStart)
      {
      ...
      stage.setX(event.getScreenX() - dragStart.getX()
                 - insetsDelta.getLeft());
      stage.setY(event.getScreenY() - dragStart.getY()
                 - insetsDelta.getTop());
        ...
        public void run(Node node, Node dragNode)
        {
          ...
          if (dragNode != node)
          {
            Event.fireEvent(node,
                            dockEnterEvent.copyFor(DockTitleBar.this,
                                                   node));
            ...
            if (dragNode != null)
            {
              ...
              // does not provide
              Event.fireEvent(dragNode,
                              dockExitEvent.copyFor(DockTitleBar.this,
                                                    dragNode));
            }
          ...
          Event.fireEvent(node,
                          dockOverEvent.copyFor(DockTitleBar.this,
                                                node));
        }
      };

      this.pickEventTarget(new Point2D(event.getScreenX(),
                                       event.getScreenY()),
                           eventTask,
                           dockExitEvent);
    }
    else if (event.getEventType() == MouseEvent.MOUSE_RELEASED)
    {
      ...
      EventTask eventTask = new EventTask()
      {
        ...
        public void run(Node node, Node dragNode)
        {
          ...
          {
            Event.fireEvent(node,
                            dockReleasedEvent.copyFor(DockTitleBar.this,
                                                      node));
          }
          Event.fireEvent(node,
                          dockReleasedEvent.copyFor(DockTitleBar.this,
